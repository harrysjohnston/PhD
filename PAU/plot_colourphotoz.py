...
delta_red = delta_norm[c2 & reds]
delta_blue = delta_norm[c2 & ~reds]
red_68ci_2 = np.percentile(delta_red, [15.9, 84.1])
blue_68ci_2 = np.percentile(delta_blue, [15.9, 84.1])

for pi in red_68ci:
	plt.axhline(pi, c='C3', ls='-', lw=2, alpha=0.8)
for pi in blue_68ci:
	plt.axhline(pi, c='C0', ls='-', lw=2, alpha=0.8)
for pi in red_68ci_2:
	plt.axhline(pi, c='C3', ls='--', lw=2, alpha=0.8)
for pi in blue_68ci_2:
	plt.axhline(pi, c='C0', ls='--', lw=2, alpha=0.8)

plt.annotate(r'$\sigma_{68}\sim%.4f (%.4f)$'%(np.diff(red_68ci)[0]/2., np.diff(red_68ci_2)[0]/2.), xy=(0.95,0.8), ha='right', xycoords='axes fraction', color='C3', fontsize=12)
plt.annotate(r'$\sigma_{68}\sim%.4f (%.4f)$'%(np.diff(blue_68ci)[0]/2., np.diff(blue_68ci_2)[0]/2.), xy=(0.05,0.8), xycoords='axes fraction', color='C0', fontsize=12)
