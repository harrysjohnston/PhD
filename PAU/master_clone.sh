...
set kcorrs_out = `echo $cat | sed 's/.fits/_zphot_2.kcorrs/g'`
...
if ( $clone == 1) then
	set call = "python clone_randoms.py $cat -magcols $appmagcols -maglims $maglims -kcorrs $kcorrs_out -randoms 1 -idcol $id -zcol $z -zlims $zmin $zmax -Nrand 40 -window 7e6 -niter 15 -load_windows 1"
	echo $call
	$call >& log_gama7e6_`ts`
endif
